import { Link } from 'react-router-dom'

import { NavButton } from '../NavButton/NavButton'
//import styles from './NavBar.module.css'
import Logo from '/tiendamia-logo.svg'
import { useState } from 'react'

export const NavBar = () => {

    const [toggle, setToggle] = useState(false)

    return (
        <>
            <header className="bg-[#ff3b3c] flex flex-col items-center py-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 lg:w-[1080px] justify-center gap-5 ">
                    <Link to="/" className="flex justify-center ">
                        <img
                            className=""
                            src={Logo}
                            width="218"
                            alt="Logo store"
                        />
                    </Link>

                    <form className="">
                        <input
                            className="py-4 text-center rounded-2xl w-[300px] "
                            type="text"
                            placeholder="Search"
                            id="search"
                        />
                    </form>

                    <ul className=" sm:col-span-2 lg:col-span-1 flex justify-evenly items-center ">
                        <li id="facebook" className="">
                            <a className="" href="https://facebook.com">
                                <img
                                    className="w-9 h-9"
                                    src="data:image/png;base64,"
                                />
                            </a>
                        </li>
                        <li id="instagram" className="">
                            <a className="" href="https://instagram.com">
                                <img
                                    className="w-9 h-9"
                                    src="data:image/png;base64,"
                                />
                            </a>
                        </li>
                        <li id="cart" className="">
                            <Link className="" to="/cart">
                                <img
                                    className="w-9 h-9"
                                    src="data:image/png;base64,"
                                />
                            </Link>
                        </li>
                    </ul>
                </div>
                <nav id="navbar" className=' flex flex-col items-center lg:w-[1080px]'>
                    <button className='w-6 h-6 block cursor-pointer md:hidden lg:hidden' onClick={()=>setToggle(!toggle)}>
                        <div className={` bg-white w-6 rounded-full h-1 mb-1 duration-700 ${toggle?"w-8 absolute left-[50%] translate-x-[-50%] rotate-45":""}`}></div>
                        <div className={` bg-white w-6 rounded-full h-1 mb-1 duration-700 ${toggle?"hidden":""}`}></div>
                        <div className={` bg-white w-6 rounded-full h-1 duration-700 ${toggle?"w-8 absolute left-[50%] translate-x-[-50%] -rotate-45":""}`}></div>
                    </button>
                    <div className={`${toggle?"block":"hidden"} md:flex lg:flex lg:w-[1080px] lg:justify-evenly `}>
                        <NavButton titleNav={'Ofertas'} linkNav={'/onsale'} />
                        <NavButton titleNav={'Cómo comprar'} linkNav={'#'} />
                        <NavButton titleNav={'Costos y tarifas'} linkNav={'#'} />
                        <NavButton titleNav={'Mis pedidos'} linkNav={'#'} />
                        <NavButton titleNav={'Garantía'} linkNav={'#'} />
                    </div>
                </nav>
            </header>
        </>
    )
}
